...
import { ITSupportForm } from "../../components/shared/ITSupportForm";
import { PersonalDataForm } from "../../components/modals/student/PersonalDataForm";
import { SchoolInfoForm } from "../../components/modals/student/SchoolInfoForm";
import { AcademicHistoryForm } from "../../components/modals/student/AcademicHistoryForm";
import { StudentNotificationSettings } from "../../components/modals/student/StudentNotificationSettings";
...
export function StudentProfile() {
  ...
  const { data: billsData } = useApi<any>("/student/bills");
  const bills = billsData?.bills ?? [];
  const student = billsData?.students?.[0];
  const tertunggakCount = bills.filter((b: any) => b.status !== "Lunas").length;
  const totalDibayar = bills
  ...
  const MENU = [
    {
      group: "Akun",
      items: [
        { icon: <User size={18} />, label: "Data Pribadi", color: "#EEF4FF", iconColor: "#1677FF", action: () => { console.log("DATA_PRIBADI_CLICKED"); setShowPersonalData(true); } },
    ...
    <div className="flex flex-col min-h-screen" style={{ background: "#F5F7FA" }}>
      ...
      >
        ...
        <div className="flex items-center gap-4">
          ...
          >
            {student?.name?.[0] ?? user?.name?.[0] ?? "B"}
          </div>
          <div>
            <p style={{ color: "white", fontSize: "1.05rem", fontWeight: 700 }}>{student?.name ?? user?.name}</p>
            <p style={{ color: "rgba(255,255,255,0.75)", fontSize: "0.8rem" }}>{student?.school_name ?? user?.school}</p>
            <div className="flex items-center gap-2 mt-1.5">
              <span
                className="px-2 py-0.5 rounded-full"
                style={{ background: "rgba(255,255,255,0.2)", color: "rgba(255,255,255,0.9)", fontSize: "0.72rem", fontWeight: 600 }}
              >
                NISN: {student?.nisn ?? user?.nisn ?? "—"}
              </span>
              {/* Terverifikasi selalu tampil jika student data ada */}
              {student && (
                <span
      ...
      <div className="px-6 -mt-4 mb-4">
        <div className="bg-white rounded-2xl p-4 shadow-sm">
          ...
          <div className="grid grid-cols-3 mb-3 pb-3" style={{ borderBottom: "1px solid #F0F0F0" }}>
            {[
              { label: "Kelas", value: student?.class_name ?? user?.class_name ?? user?.class ?? "—" },
              { label: "Status", value: "Aktif" },
              { label: "Tagihan", value: `${tertunggakCount} Tertunggak` },
            ].map((item, idx) => (
              <div
                key={item.label}
          ...
          <div className="text-center">
            <p style={{ color: "#8C8C8C", fontSize: "0.72rem" }}>Total SPP Dibayar (Tahun Ini)</p>
            <p style={{ fontWeight: 800, color: "#1677FF", fontSize: "1.1rem" }}>{formatRupiah(totalDibayar)}</p>
          </div>
        </div>
      </div>
      ...
      {/* Modals */}
      <PersonalDataForm isOpen={showPersonalData} onClose={() => setShowPersonalData(false)} user={user} student={student} />
      <SchoolInfoForm isOpen={showSchoolInfo} onClose={() => setShowSchoolInfo(false)} student={student} />
