...
import { ArrowLeft, Search, Heart, RefreshCw, AlertTriangle } from "lucide-react";
...
interface CampaignsResponse { data: Campaign[]; success: boolean; }
...
export function FundraisingPage() {
  ...
  // API returns { data: [...], success: true } — not { campaigns: [...] }
  const campaigns = data?.data ?? [];

  const filtered = campaigns.filter((c) => {
    const matchSearch = c.title.toLowerCase().includes(search.toLowerCase());
    const matchCat = activeCategory === "Semua" || c.status === activeCategory; // Assuming status or we just mock category filter
              ...
              <div className="p-4">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-1">
                    ...
                    <span style={{ fontSize: "0.78rem", color: "#8C8C8C" }}>{c.donor_count ?? 0} donatur</span>
                  </div>
                  <button
