...
import { ChevronRight, Shield, LogOut, HeadphonesIcon, User, BarChart2, Bell } from "lucide-react";
import { useAuth } from "../../context/AuthContext";
import { ITSupportForm } from "../../components/shared/ITSupportForm";
import { DonorPersonalDataForm } from "../../components/modals/donor/DonorPersonalDataForm";
import { DonationStatsForm } from "../../components/modals/donor/DonationStatsForm";
import { DonorNotificationSettings } from "../../components/modals/donor/DonorNotificationSettings";
...
export function DonorProfilePage() {
  ...
  const MENUS = [
    { icon: <User size={18} color="#595959" />, label: "Data Pribadi", sub: "Nama, email, nomor HP", action: () => setShowPersonalData(true) },
    { icon: <BarChart2 size={18} color="#595959" />, label: "Statistik Donasi", sub: "Ringkasan kontribusimu", action: () => setShowDonationStats(true) },
    { icon: <Bell size={18} color="#595959" />, label: "Notifikasi", sub: "Pengaturan pemberitahuan", action: () => setShowNotification(true) },
    { icon: <HeadphonesIcon size={18} color="#595959" />, label: "Hubungi Tim IT", sub: "Bantuan teknis aplikasi", action: () => setShowITSupport(true) },
  ];
  ...
  return (
    <div className="flex flex-col min-h-screen" style={{ background: "#F3F6FB" }}>
      ...
      {/* Modals */}
      <DonorPersonalDataForm isOpen={showPersonalData} onClose={() => setShowPersonalData(false)} user={user} />
