...
import { ChevronRight, School, MapPin, Phone, Mail, Shield, LogOut, ClipboardList, CreditCard, Bell, HeadphonesIcon } from "lucide-react";
import { useAuth } from "../../context/AuthContext";
import { ITSupportForm } from "../../components/shared/ITSupportForm";
import { SchoolDataForm } from "../../components/modals/school/SchoolDataForm";
import { AcademicYearForm } from "../../components/modals/school/AcademicYearForm";
import { BankAccountForm } from "../../components/modals/school/BankAccountForm";
import { NotificationSettings } from "../../components/modals/school/NotificationSettings";
...
export function SchoolProfilePage() {
  ...
  const MENUS = [
    { icon: <School size={16} color="#1677FF" />, label: "Data Sekolah", sub: "Edit profil & informasi", action: () => setShowSchoolData(true) },
    { icon: <ClipboardList size={16} color="#EA4E0D" />, label: "Tahun Ajaran", sub: "Kelola periode akademik", action: () => setShowAcademicYear(true) },
    { icon: <CreditCard size={16} color="#52C41A" />, label: "Rekening Sekolah", sub: "Akun penerima pembayaran", action: () => setShowBankAccount(true) },
    { icon: <Bell size={16} color="#FDD504" />, label: "Notifikasi", sub: "Pengaturan pemberitahuan", action: () => setShowNotification(true) },
    { icon: <HeadphonesIcon size={16} color="#722ED1" />, label: "Hubungi Tim IT", sub: "Bantuan teknis aplikasi", action: () => setShowITSupport(true) },
  ];
