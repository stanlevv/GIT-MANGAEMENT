...
import { Eye, EyeOff, ArrowLeft, Mail, Lock, AlertCircle, ChevronRight, HeadphonesIcon, X, MessageCircle, Phone, Send, Clock, ChevronDown, GraduationCap } from "lucide-react";
...
const DEMOS = [
  { label: "Sekolah", email: "admin@edufin.sch.id", password: "admin123", color: "#52C41A", bg: "#F6FFED" },
  { label: "Siswa/Budi", email: "budi@student.sch.id", password: "password123", color: "#1677FF", bg: "#EEF4FF" },
  { label: "Donatur", email: "rina@gmail.com", password: "password123", color: "#B07D00", bg: "#FFF7E0" },
];
...
export function LoginPage() {
  ...
  const [fieldError, setFieldError] = useState(""); // error khusus field password
  const [showHelpdesk, setShowHelpdesk] = useState(false);
  const [showForgot, setShowForgot] = useState(false);
  const [forgotEmail, setForgotEmail] = useState("");
  const [forgotSent, setForgotSent] = useState(false);

  // GoogleLogin di-handle langsung di komponen bawah
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    if (!password)     { setFieldError("Kata sandi tidak boleh kosong."); return; }
    setLoading(true);
    const result = await login(email.trim(), password);
    setLoading(false);
    if (result.success && result.role) {
      const dest = ROLE_DEST[result.role as keyof typeof ROLE_DEST] ?? "/";
      navigate(dest);
    } else {
      // Tampilkan error di bawah password field agar lebih jelas
  ...
  return (
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-6 pb-10">
        ...
        <div className="mb-5">
          <p style={{ fontSize: "0.75rem", color: "#BFBFBF", fontWeight: 600, marginBottom: "8px", letterSpacing: "0.3px" }}>
            CONTOH LOGIN — ketuk untuk isi otomatis
          </p>
          <div className="flex gap-2">
            {DEMOS.map((d) => (
              ...
              >
                ...
                <p style={{ fontSize: "0.6rem", color: "#8C8C8C", marginTop: "1px" }}>{d.password}</p>
              </button>
            ))}
          </div>
        </div>

        {/* ── Error Banner Global (koneksi) ─────────────────── */}
        {error && (
          <div className="mb-4 px-4 py-3 rounded-2xl flex items-start gap-2.5"
            style={{ background: "#FFF2EE", border: "1px solid #FFBDAD", animation: "shake 0.4s ease" }}>
        ...
        <div className="mb-2">
          ...
          <div
            ...
            style={{
              background: "#F5F7FA",
              border: `1.5px solid ${fieldError ? "#EA4E0D" : password ? "#1677FF" : "transparent"}`,
              animation: fieldError ? "shake 0.4s ease" : undefined,
            }}
          >
            <Lock size={18} color={fieldError ? "#EA4E0D" : password ? "#1677FF" : "#8C8C8C"} />
            <input
            ...
            />
            <button onClick={() => setShowPass(!showPass)} type="button" className="flex-shrink-0">
              {showPass
                ? <EyeOff size={18} color="#8C8C8C" />
                : <Eye size={18} color="#8C8C8C" />}
            </button>
          </div>
          {/* ── Pesan error inline bawah password ── */}
          {fieldError && (
            <div className="flex items-center gap-1.5 mt-2 px-1" style={{ animation: "fadeIn 0.2s ease" }}>
              <AlertCircle size={13} color="#EA4E0D" className="flex-shrink-0" />
              <p style={{ color: "#EA4E0D", fontSize: "0.78rem", fontWeight: 600 }}>{fieldError}</p>
            </div>
          )}
        </div>
        ...
        <div className="w-full flex justify-center overflow-hidden rounded-2xl" style={{ border: "2px solid #E8E8E8", boxShadow: "0 2px 8px rgba(0,0,0,0.05)" }}>
          {googleLoading ? (
            <div className="py-3.5 flex items-center justify-center gap-3">
              ...
              <span style={{ fontWeight: 600, color: "#595959", fontSize: "0.9rem" }}>Menghubungkan...</span>
            </div>
          ) : (
            <div className="w-full" style={{ minHeight: "52px", display: "flex", alignItems: "center", justifyContent: "center" }}>
              <GoogleLogin
                onSuccess={async (credentialResponse) => {
                  ...
                  try {
                    ...
                    if (result.success && result.role) {
                      navigate(ROLE_DEST[result.role as keyof typeof ROLE_DEST] || "/donor");
                    } else {
        ...
        <div className="mt-3 rounded-2xl overflow-hidden" style={{ border: "1.5px solid #E8E8E8" }}>
          ...
          <div className="flex px-3 pb-3 gap-2.5">
            ...
            >
              ...
              <div>
                <p style={{ fontSize: "0.78rem", fontWeight: 800, color: "#1A1A2E" }}>Donatur</p>
        ...
        <div className="mt-4 flex items-center justify-between px-1">
          ...
          <button
            ...
            className="px-3.5 py-1.5 rounded-full transition-all active:scale-90 flex items-center gap-1.5"
            style={{ background: "#EEF4FF", color: "#1677FF", fontSize: "0.72rem", fontWeight: 700, border: "1.5px solid #C5D8FF" }}
          >
            <GraduationCap size={14} /> Daftar NISN
          </button>
        </div>
      </div>
      ...
      <div className="fixed bottom-0 left-1/2 z-40 w-full pointer-events-none" style={{ maxWidth: 430, transform: "translateX(-50%)" }}>
        <div className="absolute bottom-6 right-6 pointer-events-auto">
          <button
            onClick={() => setShowHelpdesk(true)}
            className="flex items-center gap-2 px-4 py-3 rounded-full shadow-lg transition-all active:scale-90"
            style={{
