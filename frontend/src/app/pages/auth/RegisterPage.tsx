...
import {
  ...
  Search, AlertCircle, ChevronRight, Loader, User, Heart, GraduationCap, Info, PartyPopper, Hash, School, BookOpen, Users, MapPin, AlertTriangle, CreditCard, ClipboardList, HeartHandshake
} from "lucide-react";
...
export function RegisterPage() {
  ...
  const { register, loginWithGoogle, isAuthenticated, logout } = useAuth();
  ...
  // ── Google OAuth (Donor) ──────────────────────────────
  // Kirim access_token Google asli ke backend untuk diverifikasi
  // Backend akan hit https://www.googleapis.com/oauth2/v3/userinfo sendiri
  // GoogleLogin handler dipindah ke komponen langsung
    ...
    return (
      <div className="flex flex-col min-h-screen items-center justify-center px-8"
        ...
        <div className="relative mb-6">
          ...
          <div className="absolute -top-1 -right-1 w-8 h-8 rounded-full flex items-center justify-center text-white"
        ...
        <button
          onClick={() => isAuthenticated ? navigate(role === "siswa" ? "/student" : "/donor") : navigate("/login")}
        ...
        >
          {isAuthenticated ? "Mulai Sekarang →" : "Masuk ke Akun →"}
        </button>
      </div>
    );
  }
      ...
      <div className="flex-1 overflow-y-auto px-5 pt-6 pb-10">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex gap-2 mt-2.5 flex-wrap">
                    {[
                      { icon: <CreditCard size={12} />, label: "Bayar SPP" },
                      { icon: <ClipboardList size={12} />, label: "Cicilan" },
                      { icon: <HeartHandshake size={12} />, label: "Bantuan" }
                    ].map(t => (
                      <span key={t.label} className="px-2.5 py-0.5 rounded-full flex items-center gap-1"
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-3xl overflow-hidden"
              ...
              <div className="px-5 py-4 space-y-3" style={{ background: "white" }}>
                {[
                  { label: "NISN",           value: studentData.nisn,       icon: <Hash size={18} color="#8C8C8C" /> },
                  { label: "Sekolah",        value: studentData.school,     icon: <School size={18} color="#8C8C8C" /> },
                  { label: "Kelas",          value: studentData.class,      icon: <BookOpen size={18} color="#8C8C8C" /> },
                  { label: "Nama Orang Tua", value: studentData.parentName, icon: <Users size={18} color="#8C8C8C" /> },
                  { label: "Alamat",         value: studentData.address,    icon: <MapPin size={18} color="#8C8C8C" /> },
                ].map((row) => (
            ...
            <div className="rounded-2xl p-3.5 flex gap-2.5"
              ...
              <AlertTriangle size={18} color="#E87C00" className="mt-0.5 flex-shrink-0" />
          ...
          <div className="space-y-4">
            ...
            >
              {submitLoading ? (
                ...
                <span className="flex items-center gap-1.5">Buat Akun Sekarang <PartyPopper size={18} /></span>
              )}
            </button>
        ...
        {step === "donor-auth" && (
          <div className="space-y-4">
            ...
            <div className="w-full flex justify-center overflow-hidden rounded-2xl" style={{ border: "2px solid #E8E8E8", boxShadow: "0 2px 12px rgba(0,0,0,0.06)", background: "white" }}>
              {googleLoading ? (
                <div className="py-4 flex items-center justify-center gap-3">
                  <div className="w-5 h-5 rounded-full border-2 border-gray-300 border-t-blue-500 animate-spin" />
                  <span style={{ fontWeight: 700, color: "#242424", fontSize: "0.95rem" }}>Menghubungkan...</span>
                </div>
              ) : (
                  ...
                  <GoogleLogin
                    onSuccess={async (credentialResponse) => {
                      ...
                      try {
                        ...
                        setEmailError("Gagal terhubung ke server. Coba lagi.");
                      } finally {
            ...
            >
              {submitLoading ? (
                ...
                <span className="flex items-center gap-1.5">Daftar sebagai Donatur <Heart size={16} fill="#F95654" color="#F95654" /></span>
              )}
            </button>
