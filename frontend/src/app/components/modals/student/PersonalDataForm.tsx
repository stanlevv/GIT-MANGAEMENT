...
export function PersonalDataForm({ isOpen, onClose, user, student }: PersonalDataFormProps) {
  ...
  useEffect(() => {
    if (user || student) {
      setFormData({
        ...
        parentPhone: student?.registration_data?.initial_phone ?? "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.post("/student/update-profile", {
      ...
      <div className="w-full max-w-[430px] bg-white rounded-t-3xl p-6 pb-8 animate-slide-up max-h-[85vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 rounded-xl text-white font-bold flex justify-center items-center gap-2"
            style={{ background: "linear-gradient(135deg, #1677FF, #108EE9)", opacity: loading ? 0.7 : 1 }}
          >
