...
export function RegisterPage() {
  ...
  const handleGoogleRegister = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      setEmailError("");
      try {
        // Langsung kirim access_token ke backend (bukan fakeJwt)
        const result = await loginWithGoogle(tokenResponse.access_token);
        if (result.success) {
          // Setelah register sukses, logout agar user tidak auto-login
          // PRD: register → success screen → redirect ke /login setelah 2 detik
          logout();
          setStep("success");
        } else {
