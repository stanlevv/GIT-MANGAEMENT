...
interface CampaignSubmissionFormProps {
  isOpen: boolean;
  onClose: () => void;
  userRole: "parent" | "student" | "sekolah" | "admin_sekolah";
  studentId?: string;
}
...
export const CampaignSubmissionForm: React.FC<CampaignSubmissionFormProps> = ({
  ...
  const [formData, setFormData] = useState({
    ...
    type: "bantuan_siswa", // Default type
  });
    ...
    <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center" style={{ background: "rgba(0,0,0,0.5)" }}>
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          {/* Tipe Kampanye (Khusus Admin Sekolah) */}
          {(userRole === "sekolah" || userRole === "admin_sekolah") && (
            <div>
              <label className="block text-sm font-medium mb-2" style={{ color: "#262626" }}>Tipe Kampanye *</label>
              <select
                value={formData.type}
                ...
                style={{ background: "#FAFAFA" }}
              >
                <option value="bantuan_siswa">Bantuan Siswa</option>
                <option value="proyek_sekolah">Proyek Sekolah</option>
              </select>
            </div>
          )}
