...
export function StudentDashboard() {
  ...
  const { data: campaignsData, loading: campaignsLoading } =
    useApi<CampaignsResponse>("/campaigns");

  // Kalkulasi dari data real
  const bills     = billsData?.bills ?? [];
  const students  = billsData?.students ?? [];
  const campaigns = campaignsData?.campaigns?.slice(0, 2) ?? [];
  ...
  return (
    <div className="flex flex-col bg-white" style={{ minHeight: "100dvh" }}>
      ...
      >
        ...
        <div className="absolute top-16 -right-4 w-24 h-24 rounded-full opacity-8" style={{ background: "white" }} />
        ...
        >
          {billsLoading ? (
            ...
            <p style={{ color: "rgba(255,255,255,0.8)", fontSize: "0.85rem", textAlign: "center" }}>
      ...
      <div className="pb-24" style={{ background: "#F3F6FB" }}>
        ...
        <div className="px-5 mb-4">
          <div className="bg-white rounded-2xl overflow-hidden shadow-sm">
            {[
              { icon: "💝", label: "Ajukan Kampanye Donasi", sub: "Buat kampanye penggalangan dana", action: () => setShowCampaignForm(true) },
              { icon: "🎓", label: "Ajukan Bantuan SPP",     sub: "Gratis, tanpa bunga",             route: "/student/loan" },
              { icon: "👤", label: "Profil & Pengaturan",    sub: "Data pribadi & keamanan",          route: "/student/profile" },
            ].map((item, idx) => (
              <button
                key={item.label}
